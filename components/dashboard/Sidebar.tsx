...
import { RolCrm, useCrmAuth } from "@/components/auth/AuthGate";

const menu = [
  { label: "Resumen", href: "/dashboard", icon: LayoutDashboard, roles: ["Ventas", "Producción", "Reparto", "Caja"] as RolCrm[] },
  { label: "Pedidos", href: "/pedidos/lista", icon: ShoppingBag, roles: ["Ventas", "Producción", "Reparto", "Caja"] as RolCrm[] },
  { label: "Clientes", href: "/clientes", icon: UsersRound, roles: ["Ventas", "Reparto", "Caja"] as RolCrm[] },
  { label: "Conversaciones", href: "/conversaciones", icon: MessageCircle, roles: ["Ventas"] as RolCrm[] },
  { label: "Productos", href: "/productos", icon: Package, roles: ["Ventas", "Producción"] as RolCrm[] },
  { label: "Producción", href: "/produccion", icon: Factory, roles: ["Producción"] as RolCrm[] },
  { label: "Pendientes", href: "/productos-pendientes", icon: ClipboardList, roles: ["Producción"] as RolCrm[] },
  { label: "Cobros y FEL", href: "/finanzas", icon: WalletCards, roles: ["Ventas", "Caja"] as RolCrm[] },
  { label: "Flujo de caja", href: "/flujo-caja", icon: ReceiptText, roles: ["Caja"] as RolCrm[] },
  { label: "Reportes", href: "/reportes", icon: BarChart3, roles: ["Ventas", "Producción", "Caja"] as RolCrm[] },
  { label: "Integraciones", href: "/integraciones", icon: Boxes, roles: [] as RolCrm[] },
  { label: "Usuarios", href: "/usuarios", icon: UsersRound, roles: [] as RolCrm[] },
];
...
export default function Sidebar() {
  ...
  const menuVisible = menu.filter((item) => puedeVer(item.roles));
