...
export default function Sidebar() {
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex flex-col border-r border-slate-800 bg-slate-950 px-3 py-5 text-slate-300 transition-all duration-200 ${compact ? "w-[82px]" : "w-[264px]"} ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
      <div className={`mb-8 flex items-center ${compact ? "justify-center" : "justify-between px-2"}`}><button type="button" className="flex items-center gap-3 overflow-hidden text-left text-white" onClick={() => { setOpen(false); router.push("/dashboard"); }}><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-orange-500 font-black text-white">I</span>{!compact && <span className="whitespace-nowrap"><b className="block text-base">La Cocina de Isa</b><small className="text-xs text-slate-500">CRM gastronómico</small></span>}</button>{!compact && <button aria-label="Contraer menú" onClick={() => setCompact(true)} className="hidden rounded-lg p-2 hover:bg-white/10 lg:block"><PanelLeftClose size={18}/></button>}</div>
      <nav className="flex-1 space-y-1 overflow-y-auto">{!compact && <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[.18em] text-slate-600">Operación</p>}{menuVisible.map(({ label, href, icon: Icon }) => { const active = pathname === href || (href !== "/dashboard" && pathname.startsWith(`${href}/`)); return <button type="button" key={href} title={compact ? label : undefined} onClick={() => { setOpen(false); router.push(href); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${active ? "bg-orange-500 text-white shadow-lg shadow-orange-950/30" : "hover:bg-white/8 hover:text-white"}`}><Icon size={19} className="shrink-0"/>{!compact && <span className="whitespace-nowrap">{label}</span>}</button>; })}{rol === "Administrador" && <button type="button" title={compact ? "Usuarios" : undefined} onClick={() => { setOpen(false); router.push("/usuarios"); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${pathname.startsWith("/usuarios") ? "bg-orange-500 text-white shadow-lg shadow-orange-950/30" : "hover:bg-white/8 hover:text-white"}`}><UsersRound size={19} className="shrink-0"/>{!compact && <span className="whitespace-nowrap">Usuarios</span>}</button>}</nav>
