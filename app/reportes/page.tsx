...
type ProductoDetalle = { pedido_id: string; cantidad: number | string | null; precio: number | string | null; costo: number | string | null; productos: { nombre: string | null; categoria: string | null } | null };
...
export default function ReportesPage() {
  ...
  const cargar = useCallback(async () => {
    ...
    const [pedidosRes, pagosRes, clientesRes, detallesRes, ingredientesRes, facturasRes] = await Promise.all([
      ...
      supabase.from("pedido_detalle").select("pedido_id,cantidad,precio,costo,productos(nombre,categoria)").limit(5000),
  ...
  function exportarExcel() {
    const resumen = [
      { indicador: "Ventas registradas", valor: totalVendido }, { indicador: "Pedidos", valor: pedidosFiltrados.length }, { indicador: "Pedidos por mensajería", valor: pedidosMensajeria.length }, { indicador: "Envío cobrado por mensajería", valor: envioCobradoMensajeria }, { indicador: "Ticket promedio", valor: ticketPromedio }, { indicador: "Cobrado", valor: cobrado }, { indicador: "Saldo por cobrar", valor: saldoPendiente }, { indicador: "Margen bruto (costo guardado)", valor: margenBruto }, { indicador: "Clientes con compra", valor: clientesConCompra }, { indicador: "Facturas FEL emitidas", valor: felEmitido.length },
    ];
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-7 sm:px-7 lg:px-10"><div className="mx-auto max-w-[1600px]">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
      ["Ventas", moneda(totalVendido), `${pedidosFiltrados.length} pedidos`, BarChart3, "bg-blue-50 text-blue-800"], ["Cobrado", moneda(cobrado), "Pagos del período", CircleDollarSign, "bg-emerald-50 text-emerald-800"], ["Por cobrar", moneda(saldoPendiente), "Saldos del período", ReceiptText, "bg-amber-50 text-amber-800"], ["Margen bruto estimado", moneda(margenBruto), `${ventasProductos ? (margenBruto / ventasProductos * 100).toFixed(1) : "0.0"}% sobre ventas de productos`, TrendingUp, "bg-violet-50 text-violet-800"],
