...
export default function ReportesPage() {
  ...
  const porCanal = useMemo(() => Object.values(pedidosFiltrados.reduce<Record<string, { nombre: string; pedidos: number; ventas: number }>>((acumulado, pedido) => {
    const nombre = pedido.canal_origen || "Manual";
    const actual = acumulado[nombre] || { nombre, pedidos: 0, ventas: 0 };
    actual.pedidos += 1; actual.ventas += numero(pedido.total); acumulado[nombre] = actual; return acumulado;
  }, {})).sort((a, b) => b.ventas - a.ventas), [pedidosFiltrados]);
  ...
  function exportarExcel() {
    ...
    XLSX.utils.book_append_sheet(libro, XLSX.utils.json_to_sheet(responsablesResumen.map((responsable) => ({ responsable: responsable.nombre, pedidos: responsable.pedidos, ventas: responsable.ventas, ticket_promedio: responsable.ventas / responsable.pedidos }))), "Responsables");
    XLSX.utils.book_append_sheet(libro, XLSX.utils.json_to_sheet(porCanal), "Canales");
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-7 sm:px-7 lg:px-10"><div className="mx-auto max-w-[1600px]">
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-2" aria-label="Responsables y saldos por cobrar">
      <article className="min-w-0 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div role="region" aria-label="Tabla desplazable de responsables" tabIndex={0} className="mt-5 max-h-[34rem] overflow-auto overscroll-contain rounded-xl border border-slate-100 focus:outline-2 focus:outline-offset-2 focus:outline-orange-500">
          <table className="w-full min-w-[520px] text-sm">
            <thead className="sticky top-0 z-10 bg-white text-left text-xs uppercase text-slate-500 shadow-[0_1px_0_#e2e8f0]"><tr><th className="px-3 py-3">Responsable</th><th className="px-3 py-3 text-right">Pedidos</th><th className="px-3 py-3 text-right">Ventas</th><th className="px-3 py-3 text-right">Ticket promedio</th></tr></thead>
            <tbody>{responsablesResumen.map((responsable) => <tr key={responsable.nombre.toLocaleLowerCase("es")} className="border-b border-slate-100 last:border-0"><td className="px-3 py-3 font-bold text-slate-900">{responsable.nombre}</td><td className="px-3 py-3 text-right">{responsable.pedidos}</td><td className="px-3 py-3 text-right font-bold">{moneda(responsable.ventas)}</td><td className="px-3 py-3 text-right">{moneda(responsable.ventas / responsable.pedidos)}</td></tr>)}{!responsablesResumen.length && <tr><td colSpan={4} className="px-3 py-8 text-center text-slate-500">No hay pedidos con estos filtros.</td></tr>}</tbody>
          </table>
        </div>
      </article>
