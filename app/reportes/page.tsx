...
export default function ReportesPage() {
  ...
  const pedidosFiltrados = useMemo(() => pedidos.filter((pedido) => { const coincideEntrega = tipoEntrega === "Todos" || (tipoEntrega === "Mensajería externa" ? pedido.requiere_envio && pedido.entrega_mensajero : tipoEntrega === "Nuestro equipo" ? pedido.requiere_envio && !pedido.entrega_mensajero : !pedido.requiere_envio); return enRango(pedido.fecha_pedido || pedido.fecha_creacion, desde, hasta) && (canal === "Todos" || (pedido.canal_origen || "Manual") === canal) && (estado === "Todos" || (pedido.estado || "Sin estado") === estado) && coincideEntrega; }), [pedidos, desde, hasta, canal, estado, tipoEntrega]);
  ...
  function exportarExcel() {
    const resumen = [
      { indicador: "Ventas registradas", valor: totalVendido }, { indicador: "Pedidos", valor: pedidosFiltrados.length }, { indicador: "Pedidos por mensajería", valor: pedidosMensajeria.length }, { indicador: "Envío cobrado por mensajería", valor: envioCobradoMensajeria }, { indicador: "Ticket promedio", valor: ticketPromedio }, { indicador: "Cobrado", valor: cobrado }, { indicador: "Saldo por cobrar", valor: saldoPendiente }, { indicador: "Margen estimado (costo actual)", valor: margenEstimado }, { indicador: "Clientes con compra", valor: clientesConCompra }, { indicador: "Facturas FEL emitidas", valor: felEmitido.length },
    ];
