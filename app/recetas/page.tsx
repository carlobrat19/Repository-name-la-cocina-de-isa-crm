...
export default function RecetasPage() {
  ...
  const registrarCompra = async () => {
    ...
    const { error } = await supabase.rpc("registrar_compra_ingrediente", { p_ingrediente_id: ingredienteCompra, p_cantidad: Number(cantidadCompra), p_costo_unitario: Number(costoCompra), p_nota: notaCompra.trim() || null, p_metodo_pago: metodoCompra, p_cuenta_pago: cuentaCompra });
    ...
    setIngredienteCompra(""); setCantidadCompra(""); setCostoCompra(""); setNotaCompra(""); setMetodoCompra("Efectivo"); setCuentaCompra("Caja");
    await cargarBase();
  };
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-8 sm:px-8 lg:px-10"><div className="mx-auto max-w-7xl">
    ...
    <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_370px]">
      ...
      <aside className="space-y-7"><section className="rounded-3xl bg-slate-950 p-6 text-white shadow-sm"><p className="text-xs font-bold uppercase tracking-[.2em] text-orange-400">Resultado automático</p><h2 className="mt-2 text-2xl font-black">Costo de {productoActual?.nombre || "la receta"}</h2><div className="mt-6 space-y-3"><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-slate-300">Ingredientes</p><p className="mt-1 text-2xl font-black">{dinero(costoBase)}</p></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-slate-300">Con merma</p><p className="mt-1 text-2xl font-black">{dinero(costoConMerma)}</p></div><div className="rounded-2xl bg-orange-500 p-4"><p className="text-xs text-orange-100">Precio sugerido</p><p className="mt-1 text-2xl font-black">{dinero(precioSugerido)}</p></div></div><p className="mt-5 text-xs leading-5 text-slate-400">Al guardar, el costo con merma se sincroniza al producto. No modifica tu precio final de venta.</p></section>
        ...
        <section className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 shadow-sm"><p className="text-xs font-bold uppercase tracking-[.2em] text-emerald-700">Gastos e inventario</p><h2 className="mt-2 text-xl font-black text-slate-950">Registrar compra</h2><p className="mt-2 text-xs leading-5 text-slate-600">Suma existencias, actualiza el costo promedio y registra el gasto automáticamente en Flujo de caja.</p><div className="mt-5 space-y-3"><select value={ingredienteCompra} onChange={(event) => setIngredienteCompra(event.target.value)} className="w-full rounded-xl border border-emerald-200 bg-white p-3"><option value="">Selecciona el ingrediente comprado</option>{ingredientes.map((ingrediente) => <option key={ingrediente.id} value={ingrediente.id}>{ingrediente.nombre} · stock: {Number(ingrediente.stock_actual)} {ingrediente.unidad_base}</option>)}</select><div className="grid grid-cols-2 gap-3"><input type="number" min="0.001" step="0.001" value={cantidadCompra} onChange={(event) => setCantidadCompra(event.target.value)} placeholder="Cantidad comprada" className="rounded-xl border border-emerald-200 p-3"/><input type="number" min="0" step="0.0001" value={costoCompra} onChange={(event) => setCostoCompra(event.target.value)} placeholder="Costo por unidad" className="rounded-xl border border-emerald-200 p-3"/></div><div className="grid grid-cols-2 gap-3"><select value={metodoCompra} onChange={(event) => setMetodoCompra(event.target.value)} className="rounded-xl border border-emerald-200 bg-white p-3"><option>Efectivo</option><option>Transferencia</option><option>Tarjeta de crédito</option><option>Tarjeta débito</option><option>POS</option></select><select value={cuentaCompra} onChange={(event) => setCuentaCompra(event.target.value)} className="rounded-xl border border-emerald-200 bg-white p-3"><option>Caja</option><option>Banco</option><option>Tarjeta de crédito</option></select></div><input value={notaCompra} onChange={(event) => setNotaCompra(event.target.value)} placeholder="Proveedor o nota (opcional)" className="w-full rounded-xl border border-emerald-200 p-3"/><button type="button" disabled={registrandoCompra} onClick={() => void registrarCompra()} className="w-full rounded-xl bg-emerald-600 p-3 text-sm font-bold text-white transition hover:bg-emerald-700 disabled:opacity-60">{registrandoCompra ? "Registrando compra…" : "Registrar gasto y existencias"}</button></div></section>
      </aside>
    </div>
  </div></main>;
}
