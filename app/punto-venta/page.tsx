...
type Sucursal = { id: string; codigo: string; nombre: string; marca: string; tipo: "cocina" | "punto_venta" | "quiosco"; caja_efectivo_id: string | null };
...
export default function PuntoVentaPage() {
  ...
  const [productos, setProductos] = useState<Producto[]>([]); const [inventario, setInventario] = useState<Inventario[]>([]);
  const [turno, setTurno] = useState<Turno | null>(null); const [fondo, setFondo] = useState("");
  ...
  async function cargar() {
    ...
    const [sucursalRespuesta, productoRespuesta] = await Promise.all([
      supabase.from("sucursales").select("id,codigo,nombre,marca,tipo,caja_efectivo_id").eq("activa", true).order("nombre"),
      supabase.from("productos").select("id,nombre,categoria,precio_venta,costo,imagen_url,estado").eq("estado", "Activo").order("nombre"),
    ]);
    const error = sucursalRespuesta.error || productoRespuesta.error;
    if (error) setMensaje(`No se pudo cargar el POS: ${error.message}`);
    const nuevasSucursales = (sucursalRespuesta.data ?? []) as Sucursal[];
    setSucursales(nuevasSucursales); setProductos((productoRespuesta.data ?? []) as Producto[]);
  ...
  async function abrirCaja(event: FormEvent) {
    event.preventDefault(); if (!usuarioId || !sucursalId || !seleccionada?.caja_efectivo_id) { setMensaje("Esta sucursal no tiene una caja física configurada."); return; }
    setGuardando(true); const { error } = await supabase.from("turnos_caja_pos").insert({ sucursal_id: sucursalId, cuenta_id: seleccionada.caja_efectivo_id, cajero_id: usuarioId, fondo_inicial: Number(fondo || 0) }); setGuardando(false);
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-7 sm:px-7 lg:px-10"><div className="mx-auto max-w-[1700px]">
    ...
    {!turno ? <form onSubmit={abrirCaja} className="mb-6 rounded-3xl bg-slate-950 p-6 text-white"><div className="grid gap-4 lg:grid-cols-[1fr_260px_auto]"><div><p className="text-sm font-bold">Caja física de esta sucursal</p><div className="mt-2 rounded-xl border border-slate-700 bg-slate-800 p-3 text-sm font-semibold">{seleccionada ? `Caja · ${seleccionada.nombre}` : "Caja pendiente de configurar"}</div><p className="mt-2 text-xs text-slate-400">Las tarjetas, transferencias y POS se registran en cuentas centrales; no se mezclan con el efectivo de la sucursal.</p></div><label className="text-sm font-bold">Fondo inicial (Q)<input type="number" min="0" step="0.01" value={fondo} onChange={(event) => setFondo(event.target.value)} placeholder="0.00" className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-800 p-3 text-sm"/></label><button disabled={guardando || cargando || !seleccionada?.caja_efectivo_id} className="self-end rounded-xl bg-orange-500 px-6 py-3 text-sm font-black disabled:opacity-60">{guardando ? "Abriendo…" : "Abrir caja"}</button></div></form> : <div className="mb-6 flex justify-end"><button onClick={() => void cerrarCaja()} className="rounded-xl border border-rose-200 bg-white px-4 py-3 text-sm font-bold text-rose-700">Cerrar y cuadrar caja</button></div>}
