...
.select("rendimiento, unidad_rendimiento, merma_pct, margen_pct, iva_pct, recargo_carta_pct, costos_adicionales, receta_ingredientes(cantidad, ingredientes(nombre, unidad_base, costo_referencia, stock_actual))")
...
const costoTotalLote = conMerma + Number(recetaPiloto.costos_adicionales || 0);
const costoPorUnidad = costoTotalLote / Math.max(0.001, Number(recetaPiloto.rendimiento || 1));
const precioSinIva = costoPorUnidad / Math.max(0.01, 1 - Number(recetaPiloto.margen_pct || 0) - Number(recetaPiloto.recargo_carta_pct || 0));
...
<div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-slate-300">Costo ingredientes</p><b className="text-2xl">Q{costoBase.toFixed(2)}</b></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-slate-300">Costo total del lote</p><b className="text-2xl">Q{costoTotalLote.toFixed(2)}</b></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-slate-300">Costo por {recetaPiloto.unidad_rendimiento}</p><b className="text-2xl">Q{costoPorUnidad.toFixed(2)}</b></div><div className="rounded-2xl bg-orange-500 p-4"><p className="text-xs text-orange-100">Precio sugerido con IVA</p><b className="text-2xl">Q{precioSugerido.toFixed(2)}</b></div></div>
...
<p className="mt-4 text-xs text-slate-300">Regla: {(Number(recetaPiloto.merma_pct) * 100).toFixed(0)}% merma · {(Number(recetaPiloto.margen_pct) * 100).toFixed(0)}% margen objetivo · {(Number(recetaPiloto.iva_pct) * 100).toFixed(0)}% IVA · {(Number(recetaPiloto.recargo_carta_pct) * 100).toFixed(0)}% comisión POS/plataforma · Q{Number(recetaPiloto.costos_adicionales || 0).toFixed(2)} costos adicionales.</p>
</section>;
})()}
