...
setNombre(""); setCategoria(""); setPrecio(""); setCosto("");
...
const margenPromedioIngredientes = productosConMargenIngredientes.length ? productosConMargenIngredientes.reduce((total, producto) => total + (metricasGanancia.get(producto.id)?.porcentajeIngredientes || 0), 0) / productosConMargenIngredientes.length : 0;
const comisionPromedio = productosConMargen.length ? productosConMargen.reduce((total, producto) => total + (metricasGanancia.get(producto.id)?.porcentajeComision || 0), 0) / productosConMargen.length : 0;
...
<section className="mb-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Productos activos</p><p className="mt-2 text-3xl font-black text-slate-950">{productosActivos}</p><p className="mt-1 text-sm text-slate-500">de {productos.length} registrados</p></div>
  <div className="rounded-2xl border border-sky-100 bg-sky-50 p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-wide text-sky-700">Catálogo publicado</p><p className="mt-2 text-3xl font-black text-sky-800">{productosPublicados}</p><p className="mt-1 text-sm text-sky-700">listos para mostrar</p></div>
  <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-wide text-emerald-700">Margen completo promedio</p><p className="mt-2 text-3xl font-black text-emerald-800">{margenPromedio.toFixed(1)}%</p><p className="mt-1 text-xs text-emerald-700">después de IVA y comisión prevista</p></div>
  <div className="rounded-2xl border border-teal-100 bg-teal-50 p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-wide text-teal-700">Margen ingredientes promedio</p><p className="mt-2 text-3xl font-black text-teal-800">{margenPromedioIngredientes.toFixed(1)}%</p><p className="mt-1 text-xs text-teal-700">solo insumos · {productosConMargenIngredientes.length} recetas</p></div>
  <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-wide text-indigo-700">Comisión prevista promedio</p><p className="mt-2 text-3xl font-black text-indigo-800">{comisionPromedio.toFixed(1)}%</p><p className="mt-1 text-xs text-indigo-700">configurada en recetas · no pagos reales</p></div>
...
<button type="button" onClick={() => { limpiarFormulario(); setEditorAbierto(false); }} className="-mt-1 text-sm font-bold text-slate-500 underline sm:col-span-2">{productoEditando ? "Cancelar edición" : "Cerrar formulario"}</button>
