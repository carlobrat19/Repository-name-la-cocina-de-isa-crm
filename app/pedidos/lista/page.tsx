      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {[
            ...
            ["Por mensajería", `${resumen.mensajeria}`, Truck, "text-violet-700 bg-violet-50"],
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          {loading ? <div className="p-12 text-center text-sm text-slate-500">Cargando pedidos…</div> : pedidosFiltrados.length === 0 ? <div className="p-12 text-center"><PackageCheck className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 font-semibold text-slate-700">No encontramos pedidos con estos filtros.</p><button onClick={limpiarFiltros} className="mt-3 text-sm font-bold text-orange-600">Limpiar filtros</button></div> : <div className="max-h-[68vh] divide-y divide-slate-100 overflow-y-auto">{pedidosFiltrados.map((pedido) => {
            const items = detallesPorPedido.get(pedido.id) || [];
            const ubicacion = [pedido.departamento_entrega, pedido.municipio_entrega, pedido.zona_entrega].filter(Boolean).join(" · "); const etiquetaEntrega = pedido.requiere_envio ? (pedido.entrega_mensajero ? "Mensajería externa" : "Entrega por nuestro equipo") : "Recoger en tienda"; return <article key={pedido.id} className="group grid gap-4 px-5 py-5 transition hover:bg-orange-50/30 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_auto] lg:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-slate-100 px-2 py-1 font-mono text-[11px] font-bold text-slate-600">{pedido.codigo || "SIN CÓDIGO"}</span><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ring-inset ${statusClass(pedido.estado)}`}>{pedido.estado || "Sin estado"}</span>{pedido.pago_estado === "Pagado" ? <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700 ring-1 ring-inset ring-emerald-200">Pagado</span> : <span className="rounded-full bg-rose-50 px-2.5 py-1 text-[11px] font-bold text-rose-700 ring-1 ring-inset ring-rose-200">Pendiente de pago</span>}</div><div className="mt-3 flex items-start gap-3"><div className="rounded-xl bg-orange-100 p-2 text-orange-700"><UserRound className="h-4 w-4" /></div><div className="min-w-0"><h3 className="truncate text-base font-black text-slate-950">{pedido.cliente || "Cliente sin nombre"}</h3><p className="mt-0.5 text-sm text-slate-500">{pedido.telefono || "Sin teléfono"}</p>{pedido.direccion && <p className="mt-1 flex items-center gap-1 truncate text-xs text-slate-500"><MapPin className="h-3.5 w-3.5 text-orange-500" />{pedido.direccion}</p>}{ubicacion && <p className="mt-1 truncate pl-5 text-xs font-semibold text-orange-700">{ubicacion}</p>}</div></div></div><div className="grid grid-cols-2 gap-x-5 gap-y-3 border-y border-slate-100 py-3 text-sm lg:border-y-0 lg:py-0"><div><p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Entrega</p><p className="mt-1 font-semibold text-slate-700">{formatDate(pedido.fecha_entrega)}</p></div><div><p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Vendedor</p><p className="mt-1 font-semibold text-slate-700">{pedido.vendedor || "Sin asignar"}</p></div><div className="col-span-2"><p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Tipo de entrega</p><p className="mt-1 text-sm font-semibold text-violet-700">{etiquetaEntrega}</p></div><div className="col-span-2"><p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Productos</p><p className="mt-1 truncate text-sm text-slate-600">{items.length ? items.map((item) => `${item.cantidad || 0}× ${productosPorId.get(item.producto_id || "") || "Producto"}`).join(" · ") : "Sin productos registrados"}</p></div></div><div className="flex items-center justify-between gap-4 lg:flex-col lg:items-end"><div className="text-right"><p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Total</p><p className="mt-1 text-xl font-black text-emerald-600">{formatMoney(pedido.total)}</p><p className="mt-1 text-xs text-slate-500">{etiquetaEntrega}</p></div><Link href={`/pedidos/${pedido.id}`} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 transition group-hover:border-orange-300 group-hover:text-orange-700">Ver pedido <ChevronRight className="h-3.5 w-3.5" /></Link></div></article>;
          })}</div>}
        </section>
      </div>
    </main>
