...
router.refresh();
  ...
  function imprimirPedido() {
    ...
    ventana.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Pedido ${escapar(pedido.codigo)}</title><style>
      @page { size: A4; margin: 13mm; } * { box-sizing:border-box; } body { background:#fffdf9; color:#29211d; font:14px Arial,sans-serif; margin:0; } .document { border:1px solid #eadfd7; border-radius:18px; overflow:hidden; } .top { align-items:center; background:#3c2520; color:#fff; display:flex; justify-content:space-between; padding:25px 29px; } .brand { color:#f9c77d; font-size:11px; font-weight:800; letter-spacing:1.8px; margin:0; text-transform:uppercase; } h1 { font-family:Georgia,serif; font-size:29px; margin:7px 0 0; } .code { background:#fff8ee; border-radius:999px; color:#7c3f2b; font-size:12px; font-weight:800; padding:10px 15px; } .grid { display:grid; grid-template-columns:1.15fr .85fr; gap:10px; margin-bottom:22px; } .box { background:#fff8ee; border:1px solid #f1e4d3; border-radius:10px; min-height:76px; padding:12px 14px; } .label { color:#8d6b5b; font-size:9px; font-weight:800; letter-spacing:1.15px; text-transform:uppercase; } .value { font-size:14px; font-weight:700; line-height:1.45; margin-top:6px; } table { border-collapse:collapse; width:100%; } th { background:#f4e2cf; color:#643d2e; font-size:10px; letter-spacing:.8px; padding:11px 10px; text-align:left; text-transform:uppercase; } th:first-child { border-radius:7px 0 0 7px; } th:last-child { border-radius:0 7px 7px 0; } td { border-bottom:1px solid #eee2d8; padding:13px 10px; } .center { text-align:center; } .money { text-align:right; white-space:nowrap; } .total { display:flex; justify-content:flex-end; margin-top:19px; } .total div { background:#c76a42; border-radius:10px; color:#fff; min-width:218px; padding:13px 17px; } .total b { color:#ffe7c9; display:block; font-size:10px; letter-spacing:.8px; text-transform:uppercase; } .total strong { display:block; font-size:25px; margin-top:4px; } .notes { background:#fff8ee; border-left:4px solid #e5a662; border-radius:0 9px 9px 0; break-inside:avoid; line-height:1.6; margin-top:27px; padding:13px 16px; } .notes .label { display:block; margin-bottom:5px; } .footer { color:#9a8074; font-size:10px; margin-top:25px; text-align:center; } @media print { body { print-color-adjust:exact; -webkit-print-color-adjust:exact; } }
    </style></head><body><section class="top"><div><p class="brand">La Cocina de Isa · Pedido</p><h1>Comprobante de pedido</h1></div><div class="code">${escapar(pedido.codigo || "PEDIDO")}</div></section><section class="grid"><div class="box"><div class="label">Cliente</div><div class="value">${escapar(pedido.cliente || "Sin nombre")}<br>${escapar(pedido.telefono || "Sin teléfono")}</div></div><div class="box"><div class="label">Entrega</div><div class="value">${escapar(fecha)}<br>${escapar(pedido.hora_entrega || "Hora por confirmar")}</div></div><div class="box"><div class="label">Dirección</div><div class="value">${escapar(pedido.direccion || "Recoger en tienda")}<br>${escapar([pedido.departamento_entrega, pedido.municipio_entrega, pedido.zona_entrega].filter(Boolean).join(" · "))}</div></div><div class="box"><div class="label">Estado y pago</div><div class="value">${escapar(pedido.estado || "Pendiente")} · ${escapar(pedido.pago_estado || "Pendiente")}<br>${escapar(pedido.forma_pago || "")}</div></div></section><table><thead><tr><th>Producto</th><th class="center">Cant.</th><th class="money">Precio</th><th class="money">Subtotal</th></tr></thead><tbody>${filas}</tbody></table><div class="total"><div><b>Total del pedido</b><strong>Q${totalPedido.toFixed(2)}</strong></div></div>${pedido.observaciones ? `<section class="notes"><span class="label">Notas</span><br>${escapar(pedido.observaciones)}</section>` : ""}<p class="footer">Gracias por elegir La Cocina de Isa · Generado ${new Date().toLocaleString("es-GT")}</p><script>window.onload=()=>window.print();<\/script></body></html>`);
    ventana.document.write(`<style>.grid { margin:22px 29px; } body > table { margin:0 29px; width:calc(100% - 58px); } .total { margin-right:29px; } .notes { margin-left:29px; margin-right:29px; white-space:pre-line; } .footer { margin-left:29px; margin-right:29px; }</style>`);
