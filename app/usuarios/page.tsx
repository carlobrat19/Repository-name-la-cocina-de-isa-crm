...
import { Clock3, History, KeyRound, MailPlus, ShieldCheck, UserRound, UsersRound } from "lucide-react";
...
export default function UsuariosPage() {
  ...
  const alternar = (lista: ModuloCrm[], modulo: ModuloCrm) => lista.includes(modulo) ? lista.filter((item) => item !== modulo) : [...lista, modulo];
  const aplicarRol = (nuevo: RolCrm) => { setRol(nuevo); if (nuevo !== "Sin acceso") setSeleccion(plantillas[nuevo as keyof typeof plantillas] ?? seleccion); else setSeleccion([]); };
  async function registrar(accion: string, usuarioId: string | null, detalle: Record<string, string>) { if (administradorId) await supabase.from("auditoria_usuarios_crm").insert({ actor_id: administradorId, usuario_id: usuarioId, accion, detalle }); }

  async function crear(event: FormEvent) {
    event.preventDefault(); setGuardando("nuevo");
    const { error } = await supabase.from("invitaciones_crm").insert({ email: email.trim().toLowerCase(), nombre: nombre.trim() || null, rol, modulos: seleccion, creado_por: administradorId, acceso_hasta: accesoHasta ? new Date(accesoHasta).toISOString() : null });
    setGuardando(null); if (error) { setMensaje(error.message); return; } await registrar("Autorizó una cuenta", null, { email, rol, modulos: String(seleccion.length), vence: accesoHasta || "Sin vencimiento" });
    setNombre(""); setEmail(""); aplicarRol("Ventas"); setAccesoHasta(""); setMensaje("Cuenta autorizada. La persona ya puede activar su acceso."); await cargar();
  }
  async function actualizarPerfil(perfil: Perfil, cambios: Partial<Pick<Perfil, "rol" | "activo" | "acceso_hasta">>) {
    setGuardando(perfil.id); const { error } = await supabase.from("perfiles_crm").update(cambios).eq("id", perfil.id); setGuardando(null);
    if (error) { setMensaje(error.message); return; } await registrar("Actualizó acceso", perfil.id, Object.fromEntries(Object.entries(cambios).map(([clave, valor]) => [clave, String(valor ?? "Sin vencimiento")]))); await cargar();
  }
