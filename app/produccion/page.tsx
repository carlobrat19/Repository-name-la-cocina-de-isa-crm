...
type Pedido = { id: string; codigo: string | null; cliente: string | null; estado: string | null; vendedor: string | null; fecha_entrega: string | null; observaciones: string | null };
...
export default function ProduccionPage() {
  ...
  async function cargar() {
    const [pedidosRespuesta, detallesRespuesta, productosRespuesta] = await Promise.all([
      supabase.from("pedidos").select("id,codigo,cliente,estado,vendedor,fecha_entrega,observaciones").order("fecha_entrega", { ascending: true }).limit(250),
      supabase.from("pedido_detalle").select("pedido_id,producto_id,cantidad"),
      supabase.from("productos").select("id,nombre,categoria"),
    ]);
    if (pedidosRespuesta.error || detallesRespuesta.error || productosRespuesta.error) { console.error(pedidosRespuesta.error || detallesRespuesta.error || productosRespuesta.error); return; }
  ...
  const pedidosFiltrados = useMemo(() => pedidos.filter((pedido) => {
    ...
    return texto.includes(busqueda.toLowerCase()) && (estadoFiltro === "Todos" || pedido.estado === estadoFiltro) && (vendedorFiltro === "Todos" || pedido.vendedor === vendedorFiltro) && (!fechaFiltro || pedido.fecha_entrega === fechaFiltro);
  }), [pedidos, busqueda, estadoFiltro, vendedorFiltro, fechaFiltro]);
  ...
  const limpiar = () => { setBusqueda(""); setEstadoFiltro("Todos"); setVendedorFiltro("Todos"); setFechaFiltro(""); };

  return <main className="min-h-screen bg-slate-100 px-4 py-7 sm:px-7 lg:px-10"><div className="mx-auto max-w-[1600px]">
    <header className="mb-7 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.22em] text-orange-600">Operación de cocina</p><h1 className="mt-2 text-4xl font-black text-slate-950">Panel de producción</h1><p className="mt-2 text-sm text-slate-600">Visualiza cada pedido, actualiza sus etapas y calcula los productos que la cocina debe preparar.</p></div><div className="flex gap-3"><Link href="/pedidos/lista" className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold">Ver pedidos</Link><button onClick={() => void cargar()} className="rounded-xl bg-slate-950 px-4 py-3 text-sm font-bold text-white">Actualizar</button></div></header>
